...
# summarizeId       Summarize tracking data per id
# summarizeTrips    Summarize tracking data per trip
...
# summarizeSim.     Summarize simulated tracking data per trip
...
summarizeTrips <- function(data){
  ...
  df <- data %>%
    dplyr::arrange(date) %>%  # order by date
    dplyr::group_by(organismID, tripID) %>%  # select group info
    dplyr::summarize(date_deploy = first(date),
